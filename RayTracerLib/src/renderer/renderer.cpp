...
namespace Renderer
{
    Color Renderer::RayColor(const Ray &ray, const Color& background, const Object::Hittable &world, int depth) const
    {
        hit_record record;
        ...
        for (int j = image_height; j >= 0; j--)
        {
            ...
            for (int i = 0; i < image_width; i++)
            {
                ...
                for (int s = 0; s < imageInfo.samples_per_pixel; s++)
                {
                    ...
                    pixel_color += RayColor(r, camera.BackgroundColor(), world, imageInfo.recursion_depth);
                }
        ...
        for (int j = image_height; j >= 0; j--)
        {
            ...
            for (int i = 0; i < image_width; i++)
            {
                ...
                for (int s = 0; s < image_info.samples_per_pixel; s++)
                {
                    ...
                    Ray r = camera.GetRay(u, v);
                    pixel_color += RayColor(r,camera.BackgroundColor(), world, image_info.recursion_depth);
                }
